...
import com.jainyJobPortal.Services.UsersTypeService;
...
public class UsersController {
	...
	/*
	 * @PostMapping("/register/new") public String userRegistration(@Valid Users
	 * users, Model model) {
	 * 
	 * Optional<Users> optionalUser =usersService.getUserByEmail(users.getEmail());
	 * 
	 * if(optionalUser.isPresent()) { model.addAttribute(
	 * "error","Email already registered, try to login or register with other email."
	 * ); List<UsersType> usersTypes = usersTypeService.getAll();
	 * model.addAttribute("getAllTypes",usersTypes); model.addAttribute("user", new
	 * Users()); return "register"; }
	 * 
	 * usersService.addNew(users); return "dashboard"; }
	 */
